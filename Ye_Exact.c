...
void Refresh_DomtedSet(int G[NMAX][MMAX], int DomtedSet[], int Red[])//After adding or deleting a vertex into(from) the dominating set,
...
void Find_Dom_Set(int G[NMAX][MMAX], int Red[], int Blue[], int DomtingSet[], int DomtedSet[], int nDepthIndex,int SearchOrder[NMAX],int * nMinBound)
{
	...
	if(Can_Be_Blue(G,Blue))Find_Dom_Set(G, Red, Blue, DomtingSet, DomtedSet, nDepthIndex + 1, SearchOrder, nMinBound);//if it can be blue, I
...
int Refresh_Max_Extra(int G[NMAX][MMAX], int Red[], int Blue[], int DomtedSet[])//Used to find the maximum number of undominated vertices that can be dominated
...
int Initialise(int G[NMAX][MMAX], int Red[MMAX])//Initialization, two purpose: one is to set some vertices 'red' which must be part of the dominating set. 
